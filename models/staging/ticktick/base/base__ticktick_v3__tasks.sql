...
renamed_and_typed AS (
    SELECT
        ...
        SAFE_CAST(JSON_VALUE(raw_json, '$.sortOrder') AS INT64) AS sort_order,
        NULLIF(CAST(title AS STRING), '') AS title,
        NULLIF(CAST(content AS STRING), '') AS content,
        NULLIF(CAST(JSON_VALUE(raw_json, '$.timeZone') AS STRING), '') AS timezone,
        SAFE_CAST(JSON_VALUE(raw_json, '$.isAllDay') AS BOOL) AS is_allday,
        SAFE_CAST(JSON_VALUE(raw_json, '$.priority') AS INT64) AS priority,
        ...
        NULLIF(CAST(JSON_VALUE(raw_json, '$.repeatFlag') AS STRING), '') AS repeat_flag,
        SAFE_CAST(JSON_VALUE(raw_json, '$.startDate') AS TIMESTAMP) AS start_date,
        SAFE_CAST(JSON_VALUE(raw_json, '$.dueDate') AS TIMESTAMP) AS due_date,
        ...
        NULLIF(CAST(JSON_VALUE(raw_json, '$.parentId') AS STRING), '') AS parent_id,
        ARRAY(
            SELECT item
...
cast_tz AS (
    SELECT
        * EXCEPT (start_date, due_date, _completed_time, created_time, modified_time, updated_at),
